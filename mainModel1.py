...
RPC.update(state="Converting songs", details="BeatSaber to ADHS*MASTER converter",large_image="avatar",buttons=[{"label": "GitHub", "url": "https://google.de"}])
...
def openFile():
    ...
    config.set('Map', 'songLength', str(inputSongLen.get()))
    config.set('Map', 'converted',str(1))
    #config.set('Map', 'notesRight', str(notesRight))

    # Save to a file
    outputfile = "map_{}.ini".format(inputName.get())
    try:
